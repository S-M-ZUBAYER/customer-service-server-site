...
router.put('/eventProductImages/update/:id', upload.single('newProductImg'), (req, res) => {
  ...
  const sql = `
    ...
    SET productName=?, productPrice=?, productDescription=?, modelNumber=?, printerColor=?, connectorType=?, 
        stockQuantity=?, shelfStartTime=?, shelfEndTime=?, afterSalesText=?, afterSalesInstruction=?, 
        inventoryText=?, productImg=? 
    WHERE id=?`;

  updateDatabase(sql, [productName, productPrice, productDescription, modelNumber, printerColor, connectorType,
