...
// Get version info
router.get('/powerBank/allVersion', async (req, res) => {
    const query = 'SELECT * FROM powerBankVersion';
    try {
...
// Get version info
router.get('/powerBank/version/global', async (req, res) => {
    const query = 'SELECT * FROM powerBankVersion WHERE region = ?';
    try {
...
router.get('/powerBank/version/china', async (req, res) => {
    const query = 'SELECT * FROM powerBankVersion WHERE region = ?';
    try {
...
// Create version info
router.post('/powerBank/version/add', async (req, res) => {
    ...
    const query = `INSERT INTO powerBankVersion (modelName, region, versionCategory, sliderImageMark) VALUES (?, ?, ?, ?)`;

    try {
        const result = await executeQuery(query, [modelName, region, versionCategory, sliderImageMark]);
        res.json({
            status: 'success',
            message: 'Power Bank Version info created successfully.',
            result
        });
    } catch (err) {
...
// Update version info
router.put('/powerBank/version/update/:id', async (req, res) => {
    const { id } = req.params;
    const { modelName, region, versionCategory, sliderImageMark } = req.body;
    ...
    try {
        ...
        res.json({
            status: 'success',
            message: 'Power Bank Version info updated successfully.',
            result
        });
    } catch (err) {
...
// Delete version info
router.delete('/powerBank/version/delete/:id', async (req, res) => {
    const { id } = req.params;
    ...
    const query = `DELETE FROM powerBankVersion WHERE id = ?`;
    ...
    try {
        ...
        res.json({
            status: 'success',
            message: 'Power Version info deleted successfully.',
            result
        });
    } catch (err) {
